  ...
  <div id="product-tabs-slider" class="scroll-tabs outer-top-vs wow fadeInUp">
    <div class="more-info-tab clearfix ">
      ...
      <ul class="nav nav-tabs nav-tab-line pull-right" id="new-products-1">
        <li><a data-transition-type="backSlide" href="{{ route('all.product') }}" >All</a></li>
        @foreach ($category as $item)
        @php
        $cat=App\Model\Category::where('id',$item->cat_id)->first();
        @endphp
        <li><a data-transition-type="backSlide" href="{{ route('category.product',$cat->name) }}">{{ $cat->name }}</a></li>
        @endforeach
      </ul>
      <!-- /.nav-tabs -->
    </div>
